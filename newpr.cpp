...
void fastestPathToNeigh(City* curCity, City* mainCity, City**& cities, Information info, bool visitedTowns, int time) {
    if (curCity != nullptr) {
       ...
       // cout << cities[mainCity->cityNum]->name << endl;
        ...
        if (curCity->neighbour != nullptr) {
            fastestPathToNeigh(curCity->neighbour, curCity, cities, info, visitedTowns, time);
        }
...
//    }
//}
//void showVisitedTowns(City** cities, Information info, Requests req, City* city) {
//
//    if (city != nullptr) {
//
//        if (city->from != -1) showVisitedTowns(cities, info, req, cities[city->from]);
//
//        if (city->from != -1 && !checkIfWordsEqual(req.destination, cities[city->from]->name) &&
//            !checkIfWordsEqual(req.source, cities[city->from]->name)) cout << " " << city->name;
//
//    }
//}
...
void showVisitedTowns(City** cities, Information info, Requests req, City* city) {
    ...
    if (city != nullptr) {
        ...
        if (city->from != -1) showVisitedTowns(cities, info, req, cities[city->from]);
        if (city->from != -1 && !checkIfWordsEqual(req.destination, cities[city->from]->name)
            && !checkIfWordsEqual(req.source, cities[city->from]->name)) cout << " " << cities[city->from]->name;
    }
}

void findLastCities(City* source, City* curCity, Coords pos, int basicTime, Information& info, int counter, City**& cities, bool change) {
    ...
    if (counter < 2) {
        ...
        if (onTheMap(info, pos.x + 1, pos.y) && source->roadMap[pos.y][pos.x + 1] == basicTime - 1) {
            pos.x += 1;
            basicTime--;

        }
        else if (onTheMap(info, pos.x - 1, pos.y) && source->roadMap[pos.y][pos.x - 1] == basicTime - 1) {
            pos.x -= 1;
            basicTime--;

        }
        else if (onTheMap(info, pos.x, pos.y + 1) && source->roadMap[pos.y + 1][pos.x] == basicTime - 1) {
            pos.y += 1;
            basicTime--;
        ...
        findLastCities(source, curCity, pos, basicTime, info, counter, cities, change);
        ...
        //sprawdzenie lotu
        for (int i = 0; i < info.flightsNum; i++) {
            if (checkIfWordsEqual(info.flights[i].destination, curCity->name)) {
                //cout << "znaleziony lot do "<< info.flights[i].destination <<" z "<< info.flights[i].source << endl;
                //sprawdzenie czasow
                City* source = findCity(info.flights[i].source, cities, info);
                City* destination = findCity(info.flights[i].destination, cities, info);
                //cout << "dest: " << c->name << " source " << c2->name << endl;
               // if (source->roadMap[source->coords.y][source->coords.x] < basicTime) {
                    //cout << "zmiana lotu" << endl;
                    cities[curCity->cityNum]->from = source->cityNum;
                   // cities[curCity->cityNum]->from--;
                    change = false;
                //}
        ...
        if (change == true) {
            ...
            cities[curCity->cityNum]->from = previousCity;
        }
...
void getFastestPath(Information info, City**& cities, City*& head, Requests req, int lastIndex) {
    ...
    if (head != nullptr) {
        ...
        fastestPathToNeigh(cities[lowestIndex], cities[lowestIndex], cities, info, req.mode, cities[lowestIndex]->timeFromSource);
        ...
        for (int i = 0; i < info.citiesCounter; i++) {
            //cout << cities[i]->name << " " << head->roadMap[cities[i]->coords.y][cities[i]->coords.x];
            
            if (!checkIfWordsEqual(head->name, cities[i]->name)) {
                //cout << "sprawdzam teraz: " << cities[i]->name << endl;
                findLastCities(head, cities[i], cities[i]->coords, head->roadMap[cities[i]->coords.y][cities[i]->coords.x], info, 0, cities, true);
            }
                
        }
    }
    /*for (int i = 0; i < info.citiesCounter; i++) {
        cout << cities[i]->name << " " << cities[i]->from << endl;
    }
    getchar();
    getchar();*/
    /*for (int i = 0; i < info.citiesCounter; i++) {
        wypiszMiasta(info, cities[i]);
    }*/
    //getchar();
    //getchar();
    ...
    if (req.mode == true && findCity(req.destination, cities, info) != nullptr && findCity(req.destination, cities, info)->timeFromSource != -1
        && findCity(req.destination, cities, info)->timeFromSource != 0) {
        // cout << req.source << " " << req.destination << " " << req.mode << " ";
        cout << findCity(req.destination, cities, info)->timeFromSource;
        showVisitedTowns(cities, info, req, findCity(req.destination, cities, info));
        cout << endl;
    }

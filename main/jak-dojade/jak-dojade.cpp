...
void findFlightIn(Flights f, City* city, bool& overwrite, City**& cities, Information& info) {
    

    if (city != nullptr && city->coords.x == f.pos.x && city->coords.y == f.pos.y) {
        overwrite = true;
        if (f.time < city->timeToNeighbour) {
            if(city->cityNum != -1)cout << cities[city->cityNum]->name;
            //cout << " z " << findCity(f.source, cities, info)->name<<endl;
           // city->from = findCity(f.source, cities, info)->cityNum;
            city->timeToNeighbour = f.time;
        }
    }
    else if (city != nullptr) {
        findFlightIn(f, city->neighbour, overwrite, cities, info);
    }
}


void addFlight(Flights f, City*& city, City**& cities, Information& info) {
    ...
    if (city == nullptr) {
       ...
       // newCity->from = findCity(f.source, cities, info)->cityNum;
        city = newCity;

    }
    else addFlight(f, city->neighbour, cities,info);
}

void addFlights(Information& info, City*& city, City**& cities) {
    bool overwrite = false;
    for (int i = 0; i < info.flightsNum; i++) {
        if (city != nullptr && checkIfWordsEqual(info.flights[i].source, city->name)) {
            if (city != nullptr && city->neighbour != nullptr)
                findFlightIn(info.flights[i], city->neighbour, overwrite, cities, info);
            if (overwrite == false) {
             
                addFlight(info.flights[i], city, cities, info);
            }

        }
    }
}
//void findInList(Coords pos, City*& source, int previousCity) {
//    if (source!= nullptr && pos.x == source->coords.x && pos.y == source->coords.y)
//        source->from = previousCity;
//    else findInList(pos, source->neighbour, previousCity);
//}
//void findPreviousCity(int**& roadMap, Coords pos, Coords basicPos, Information& info, int counter, int basicTime, City**& cities, City* source) {
...
//
//
//        if (info.map[pos.y][pos.x] == CITY) {
//            counter++;
//            //jesli jestem w miescie i lot ma destination tu gdzie jestem i source z mniejsza iloscia do source to dodaj previous
//            //if (counter == 1) {
//
//            //
//            //for (int i = 0; i < info.flightsNum; i++) {
//            //    City* c = findCity(info.flights[i].destination, cities, info);
//            //    City* c2 = findCity(info.flights[i].source, cities, info);
//            //    /*if (findCity(info.flights[i].destination, cities, info)->coords.x == pos.x &&
//            //        findCity(info.flights[i].destination, cities, info)->coords.y == pos.y && 
//            //        findCity(info.flights[i].)*/
//            //    if (c->coords.x == pos.x && c->coords.y == pos.y && info.map[c2->coords.y][c2->coords.x] < basicTime - 1)
//            //    {
//            //        cout << "Flight from " << c2->name << "Do " << c->name << endl;
//            //        int basicCity = findCitiesNum(basicPos, cities, info);
//            //        pos.x = c->coords.x;
//            //        pos.y = c->coords.y;
//            //        int previousCity = findCitiesNum(pos, cities, info);
//            //        //int previousCity = c->cityNum;
...
//            /*getchar();
//            getchar();*/
//            findPreviousCity(roadMap, pos, basicPos, info, counter, basicTime, cities, source);
//
//        }
//        else {
...
//            //int basicCity = findCitiesNum(basicPos, cities, info);
//            int previousCity = findCitiesNum(pos, cities, info);
//            findInList(pos, source, previousCity);
//
...
void checkRoadsAround(Information& info, Queue*& q, int**& roadMap, Queue*& neighToLink, City* city) {
    ...
    if (onTheMap(info, front(q)->coords.x + 1, front(q)->coords.y) && !visited(roadMap, newPos)) {
        ...
        if (info.map[front(q)->coords.y][front(q)->coords.x + 1] == ROAD) {
            ...
            addToQueue(neigh, q->road, city->cityNum, neighToLink, neighToLink);
            addToQueue(newPos, q->road, city->cityNum, q, q);
...
City** createGraph(Information& info) {
    ...
    for (int i = 0; i < info.citiesCounter; i++) {
        if (info.flightsNum > 0)addFlights(info, cities[i], cities);
    }
    for (int i = 0; i < info.citiesCounter; i++) {
    ...
    /*for (int i = 0; i < info.flightsNum; i++) {
        findCity(info.flights[i].source, cities, info).
    }*/

    return cities;
}

void wypiszMiasta(Information& info, City* city, City** cities) {

    if (city != nullptr) {
        //cout << " " << city->name << " - from ";
        //if(city->from != -1)cout<<cities[city->from]->name;
        wypiszMiasta(info, city->neighbour, cities);
    }
    /*
    for (int i = 0; i < info.citiesCounter; i++) {
...
void fastestPathToNeigh(City* curCity, City* mainCity, City**& cities, Information info, int time, bool visitedTowns) {
    if (curCity != nullptr) {
        //cout << cities[curCity->cityNum]->name << " ";
       // cout << cities[mainCity->cityNum]->name << endl;
        
        int index = curCity->cityNum;
        if (cities[index]->visited == false && cities[index]->timeFromSource > curCity->timeToNeighbour + time
            || cities[index]->timeFromSource == -1) {
            cities[index]->timeFromSource = curCity->timeToNeighbour + time;
            //cities[index]->visited = true;
            //cout << time << endl;
            //cities[index]->from = cities[]->cityNum;
        }
        if (curCity->neighbour != nullptr) {
            fastestPathToNeigh(curCity->neighbour, curCity, cities, info, time, visitedTowns);
        }
...
void findPreviousCities(City** cities, City *source) {
...
void getFastestPath(Information info, City**& cities, City*& head, Requests req, int lastIndex) {
    ...
    if (head != nullptr) {
        ...
        //for (int i = 0; i < info.citiesCounter; i++) {
            ...
            fastestPathToNeigh(cities[lowestIndex], cities[lowestIndex], cities, info, cities[lowestIndex]->timeFromSource, req.mode);


            cities[lowestIndex]->visited = true;
          //  if (findCity(req.destination, cities, info)->visited == true)break;
        //}

            //findPreviousCities(cities, )
            //for (int i = 0; i < info.citiesCounter; i++)
                //findPreviousCity(head->roadMap, cities[i]->coords, cities[i]->coords, info, 0, 0, cities, head);
...
void dijkstra(City**& cities, Information info) {
    ...
    for (int i = 0; i < info.reqNum; i++) {
       ...
       for (int i = 0; i < info.citiesCounter; i++) {
           wypiszMiasta(info, cities[i], cities);
           cout << endl;
       }

       /*for (int i = 0; i < info.citiesCounter; i++) {
           cout << cities[i]->name << " poprzednik: ";
           if(cities[i]->from != -1 )cout<< cities[cities[i]->from]->name << endl;
       }
...
int main()
{
    ...
    //       // cout << city[ind]->name;
    //       if (city[i]->previousCity[j] == -55) cout << city[j]->name << " -55" << endl;
    //       else if(city[i]->previousCity[j] != -1) cout << city[j]->name << " " << city[city[i]->previousCity[j]]->name << endl;
